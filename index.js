...
async function run() {
  try {
    ...
    app.post("/reserve", async (req, res) => {
      ...
      const result = await reservationCollection.insertOne(booked);
      res.send(result);
    });
    ...
    app.delete("/reserve/:id", verifyToken, verifyAdmin, async (req, res) => {
      const id = req.params.id;
      const query = { _id: new ObjectId(id) };
    ...
    app.post("/payments", async (req, res) => {
      ...
      // carefully delete each item form the card
      const query = {
        _id: {
